...
public abstract class GasPoweredCar extends Car {
    ...
    public void refillTank(double gallons) {
        ...
        if (this.fuelLevel + gallons > this.fuelCapacity + 0.1) {
            throw new IllegalArgumentException(String.format("Adding %.1f gallons to the tank while it's fuel level " + "is %.1f gallons would cause it to overflow (%.1f > %.1f)", gallons, getFuelLevel(), (gallons + getFuelCapacity()), getFuelCapacity()));
        } else if (gallons < 0) {
